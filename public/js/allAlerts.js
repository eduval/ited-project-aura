...
function renderFullAlertItem(alert) {
  ...
  return `<div class="card mb-2 alert-card"><div class="card-body p-2"><div class="d-flex justify-content-between"><h6 class="card-title mb-1">${
    alert.title || "Alert"
  }</h6><span class="badge ${
    alert.read
    ...
    alert.read ? "Read" : "New"
  }</span></div><p class="card-text small mb-1">${
    alert.message || ""
  }</p><div class="d-flex align-items-center text-muted smaller mt-1"><i class="fi fi-robot me-1"></i><span>Issued by: <strong>${
    alert.issuedBy || "System"
  }</strong> &bull; ${formatTimestamp(
    alert.timestamp
  )}</span></div>${commentsHTML}<div class="mt-2 d-flex flex-wrap gap-1 align-items-center border-top pt-2"><button class="btn btn-sm ${readButtonClass} mark-as-read-btn" data-key="${
    alert.key
  }" data-read="${
    alert.read
  }">${readButtonText}</button><button class="btn btn-primary btn-sm add-comment-btn" data-bs-toggle="modal" data-bs-target="#commentModal" data-key="${
    alert.key
