...
function publicUrl(originFolder, relPath) {
    const cleanRel = String(relPath || "").replace(/^\/+/, ""); // trim leading slash
    const cleanOrigin = String(originFolder || "").replace(/^\/+/, "");
    return CDN_BASE + encodeURI(cleanOrigin) + "/" + encodeURI(cleanRel);
}
...
async function buildRows() {
    ...
    for (const [batchId, batchVal] of batchEntries) {
        ...
        const efEntries = Object.entries(wordFiles).sort((a, b) =>
        ...
        for (const [key, file] of efEntries) {
            ...
            const downloadURL = rel ? publicUrl(origin, rel) : "";
